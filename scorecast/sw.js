/* ============================================================
   ...
   ============================================================ */
const CACHE_NAME = 'scorecast-cache-v21';
const APP_SHELL = [
  '/',
  'index.html',
  'app.html',
  ...
  'js/equipos-info.js',
  'js/tablas-cuadros.js',
  'img/escudos/betplay/dimayor.png',
  'img/escudos/premier/epl.png',
  'img/escudos/laliga/laliga.png',
  'img/escudos/bundesliga/bundesliga.png',
  'img/escudos/champions/ucl.png',
  'img/escudos/libertadores/libertadores.png',
  'img/escudos/sudamericana/sudamericana.png',
  'manifest.json',
  'icons/icon-192.png',
  'icons/icon-512.png',
  'sounds/gol.mp3'
    ...
    fetch(event.request)
      ...
      .catch(() => {
        ...
        return caches.match(event.request).then(cachedResponse => {
          ...
          // Si es una página de navegación, devolver index.html (SPA-like)
          if (event.request.mode === 'navigate') {
            return caches.match('index.html');
          }
          // Si no hay nada en caché, devolver un error 503 personalizado
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    data = { titulo: 'ScoreCast', cuerpo: event.data ? event.data.text() : 'Novedad en ScoreCast' };
  }

  const titulo = data.titulo || '⚽ ScoreCast';
  const cuerpo = data.cuerpo || data.body || '¡Hay novedades en ScoreCast!';
  const icono = data.icon || 'icons/icon-192.png';
  const badge = data.badge || 'icons/icon-192.png';
  const tag = data.tag || 'scorecast-push-' + Date.now();
    ...
    clients.matchAll({ type: 'window', includeUncontrolled: true })
      .then(windowClients => {
        ...
        for (const client of windowClients) {
          if (client.url.includes('scorecast') || client.url.includes('app.html')) {
    ...
    case 'notificar':
      if (event.data.titulo && event.data.cuerpo) {
        self.registration.showNotification(event.data.titulo, {
          ...
          tag: event.data.tag || 'scorecast-msg-' + Date.now(),
